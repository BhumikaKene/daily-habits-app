...
export const chatCompletion = async ({ system, user, temperature = 0.7 }) => {
  ...
  if (!c) {
    return {
      ok: false,
      content:
        "AI features are disabled – set GEMINI_API_KEY in the backend .env to enable real AI responses. Meanwhile here is a",
    };
  }
...
export const SYSTEM_PROMPTS = {
  ...
  recovery:
    "You are a compassionate habit recovery coach. The user broke a streak. Write a 3-day recovery plan tailored to this specific habit. Be warm but actionable. Use this Structure: short empathetic opening (1-2 sentences), then Day1 / Day 2 / Day 3 sections with one concrete action each, then a closing line of encouragement. 150-220 words total.",
  chat:
    "You are a helpful habit analysis assistant. Answer the user's question using ONLY the provided habit data as context. Be specific - cite actual habit names, days, percentages. Keep replier under 120 words. If the data is insufficient, say so briefly.",
  morning:
